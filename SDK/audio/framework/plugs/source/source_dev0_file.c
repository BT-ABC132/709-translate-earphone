...
#define OUTPUT_BUFF_SIZE    (2048*5)//(2048)//
static u8 app_send_opus_flow=0;
static u8 app_send_opus_cnt=0;
...
static u8 source_dev0_cbuf_data_flow_en(void)
{
    if(cbuf_get_data_len(&output_cbuf_h) >= OUTPUT_BUFF_SIZE*8/10){
        return 1;
    }
    return 0;
}
static u8 source_dev0_cbuf_data_clear_flow(void)
{
    if(cbuf_get_data_len(&output_cbuf_h) <= OUTPUT_BUFF_SIZE*3/10){
        return 1;
    }
    return 0;
}
...
{
    if(output_buff == NULL){
        printf("---source_input_write null\n");
        return 0;
    }
    u32 ret = cbuf_write(&output_cbuf_h, data, len);
    if(ret != len){
        printf("---source_input_write len_err\n");
        putchar('N');
    }
    if(source_dev0_cbuf_data_flow_en()){
        if(app_send_opus_flow == 0){
            app_send_opus_flow=1;
            app_send_opus_cnt=0;
            app_send_message(APP_MSG_HJX_APP_SEND_OPUS_FLOW, app_send_opus_flow);
        }/*else{
            app_send_opus_cnt++;
            if(app_send_opus_cnt>=3){
                app_send_opus_cnt=0;
                app_send_message(APP_MSG_HJX_APP_SEND_OPUS_FLOW, app_send_opus_flow);
            }
        }*/
...
{
    if(output_buff == NULL){
        printf("---source_input_read null\n");
        return 0;
    }
    if(source_dev0_cbuf_data_clear_flow()){
        if(app_send_opus_flow){
            app_send_opus_flow=0;
            app_send_opus_cnt=0;
            app_send_message(APP_MSG_HJX_APP_SEND_OPUS_FLOW, app_send_opus_flow);
        }/*else{
    ...
    if(len > cbuf_get_data_len(&output_cbuf_h)){
        printf("---source_input_read len_err len:%d cbuf len:%d\n",len,cbuf_get_data_len(&output_cbuf_h));
...
static u8 *source_dev0_get_packet(struct source_dev0_file_hdl *hdl, u32 *len)
{
    ...
    if(data_ok == 0 && cbuf_get_data_len(&output_cbuf_h) < 500/*(OUTPUT_BUFF_SIZE / 4)*/){
        return NULL;
    }
    data_ok = 1;
    //do something
    ...
    if(packet_len == 0){
        printf("---source_input_read no_data\n");
        putchar('K');
        data_ok = 0;
    }
    *len = packet_len;
    //printf("---source_dev0_len:%d\n",packet_len);
