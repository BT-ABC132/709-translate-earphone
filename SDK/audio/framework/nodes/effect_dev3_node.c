...
extern cbuffer_t hjx_opus_decode_cbuffer;
extern u8 *hjx_opus_decode_cache_buf;
/* 自定义算法，初始化
...
static u32 audio_effect_dev3_run(struct effect_dev3_node_hdl *hdl, s16 *indata, s16 *outdata, u32 indata_len)
{
...
#endif
    //do something
    //printf("---role:%d %d\n",tws_api_get_role(),hjx_flag_info.opus_studio_num);
    if(source_dev1_get_idx_enable(0) == 0){
        //printf("---effect_dev3_null\n");
        return indata_len;
    }
    u8 *data = (u8 *)indata;
    ...
    hdl->src_busy = 0;
    if(hjx_flag_info.opus_studio_num == HJX_OPUS_STUDIO_AUDIO_AND_VIDEO){
    ///此场景不需要发当前通道数据，填0
    //printf("---clear_mic:%d\n",data_len);
    ...
    || (hjx_flag_info.opus_studio_tws_status == 1)){
        ...
        if(hjx_flag_info.opus_studio_num == HJX_OPUS_STUDIO_CALL){
            //printf("---clear_effect_dev3\n");
            memset(outdata,0,indata_len);///清这个，后面的数据流节点没有声音
            if (hjx_opus_decode_cache_buf) {
                u32 cache_len = cbuf_get_data_len(&hjx_opus_decode_cbuffer);
                //printf("---cache_len:%d %d\n",cache_len,indata_len);
                if (cache_len >= indata_len) {
                    cbuf_read(&hjx_opus_decode_cbuffer, outdata, indata_len);
                }
            }
        }
    }
