 ...
 * 非0 : 按照帧长输入数据到算法处理接口
 */
#define EFFECT_DEV0_FRAME_POINTS  0 //(320)//(256)
...
cbuffer_t hjx_opus_decode_cbuffer;
u8 *hjx_opus_decode_cache_buf = NULL;
#define HJX_OPUS_DECODE_BUFF_SIZE    (640*5)//单声道一帧PCM 640byte
...
{//printf("---audio_effect_dev0_init\n");
    //do something
    if (!hjx_opus_decode_cache_buf) {
        int cache_buf_len = HJX_OPUS_DECODE_BUFF_SIZE; 
        //申请cbuffer
        hjx_opus_decode_cache_buf = zalloc(cache_buf_len);
        if (hjx_opus_decode_cache_buf) {
            cbuf_init(&hjx_opus_decode_cbuffer, hjx_opus_decode_cache_buf, cache_buf_len);
        }
    }
}
...
{//printf("---audio_effect_dev0_run\n");
    ...
    if(hdl == NULL){
        //printf("---effect_dev0_null\n");
        return indata_len;
    }
    u8 ch_num = hdl->dev.in_ch_num;
    //printf("---effect_dev0 in_ch_num:%d out_ch_num:%d sample_rate:%d\n",hdl->dev.in_ch_num,hdl->dev.out_ch_num,hdl->dev.sample_rate);
    if(ch_num == 2){
	...
	//memset(indata,0,indata_len);///清这个，后面的数据流节点没有声音
    ...
    || (hjx_flag_info.opus_studio_tws_status == 1)){
        if(hjx_flag_info.opus_studio_num == HJX_OPUS_STUDIO_CALL){
            printf("---clear_effect_dev0\n");
            //memset(outdata,0,indata_len);///清这个，后面的数据流节点没有声音
            ///加保存indata数据
            if (hjx_opus_decode_cache_buf) {
                int wlen = 0;
                wlen = cbuf_write(&hjx_opus_decode_cbuffer, indata, indata_len);
                if (wlen != indata_len) {
                    printf("---wlen:%d %d\n",wlen,indata_len);
                }
            }
...
{//printf("---audio_effect_dev0_exit\n");
    //do something
    if(hjx_opus_decode_cache_buf){
        free(hjx_opus_decode_cache_buf);
        hjx_opus_decode_cache_buf = NULL;    
    }    
}
...
{//printf("---effect_dev0_handle_frame\n");
...
{//printf("---effect_dev0_adapter_bind\n");
...
static int effect_dev0_adapter_ioctl(struct stream_iport *iport, int cmd, int arg)
{
    int ret = 0;
    struct effect_dev0_node_hdl *hdl = (struct effect_dev0_node_hdl *)iport->node->private_data;
    //printf("---effect_dev0_adapter_ioctl:0x%x\n",cmd);
    switch (cmd) {
