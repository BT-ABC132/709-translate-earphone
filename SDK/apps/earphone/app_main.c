...
static void test_printf(void *_arg)
{
    ...
    printf(">tws connect:%d role:%d call:%d studio:%d\n", get_bt_tws_connect_status(),role,esco_player_runing(),hjx_flag_info.opus_studio_num);   //打印tws主从

    if (hjx_ble_connect_flag == 0) {
        printf("---ble_discon\n");
    }else{
        printf("---ble_connect\n");
    }
...
static void app_task_loop(void *p)
{
	...
	extern void hjx_flag_info_reset_fun(void);
    hjx_flag_info_reset_fun();
