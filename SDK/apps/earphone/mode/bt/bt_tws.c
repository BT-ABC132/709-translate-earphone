...
static u8 set_channel_by_code_or_res(void)
{
    ...
    if (channel) {
        hjx_flag_info.tws_channel=channel;
...
u8 hjx_get_channel_by_code_or_res(void)
{///ble广播需要广播TWS左右声道，set_channel_by_code_or_res函数调用在ble初始化之后，重新写个函数，根据电阻等获取左右声道
    u8 count = 0;
    char channel = 0;
#if (CONFIG_TWS_CHANNEL_SELECT == CONFIG_TWS_EXTERN_UP_AS_LEFT) || \
    (CONFIG_TWS_CHANNEL_SELECT == CONFIG_TWS_EXTERN_UP_AS_RIGHT)
...
int bt_tws_poweron()
{
    ...
    if (err == 0) {
        ...
        if(hjx_flag_info.tws_channel == 0){
            hjx_flag_info.tws_channel=tws_api_get_local_channel();
        }
        printf("--tws_channel:0x%x 0x%x 0x%x\n",hjx_flag_info.tws_channel,bt_tws_get_local_channel(),tws_api_get_local_channel());
...
void hjx_bt_page_scan_for_test(u8 inquiry_en)
{
    u8 local_addr[6];
...
#define TWS_FUNC_ID_BLE_MAC_SYNC    TWS_FUNC_ID('L', 'M', 'A', 'C')

static void bt_tws_ble_mac_sync_fun(void *_data, u16 len, bool rx)
{
    u8 *data = (u8 *)_data;
    if (rx) {
        hjx_ble_mac_buf2[0] = data[0];
        hjx_ble_mac_buf2[1] = data[1];
        hjx_ble_mac_buf2[2] = data[2];
        hjx_ble_mac_buf2[3] = data[3];
        hjx_ble_mac_buf2[4] = data[4];
        hjx_ble_mac_buf2[5] = data[5];
        printf("---hjx_ble_mac_buf2:");
        put_buf(hjx_ble_mac_buf2,6);
        app_send_message(APP_MSG_HJX_VM_BLE_MAC2, 0);
    }
}
REGISTER_TWS_FUNC_STUB(app_ble_mac_sync_stub) = {
    .func_id = TWS_FUNC_ID_BLE_MAC_SYNC,
    .func    = bt_tws_ble_mac_sync_fun,
};

void bt_tws_sync_ble_mac()
{
    u8 data[6];

    data[0] = hjx_ble_mac_buf[0];
    data[1] = hjx_ble_mac_buf[1];
    data[2] = hjx_ble_mac_buf[2];
    data[3] = hjx_ble_mac_buf[3];
    data[4] = hjx_ble_mac_buf[4];
    data[5] = hjx_ble_mac_buf[5];
    if (TWS_ROLE_MASTER == tws_api_get_role()) {
        tws_api_send_data_to_slave(data, 6, TWS_FUNC_ID_BLE_MAC_SYNC);
    }else{
        tws_api_send_data_to_sibling(data, 6, TWS_FUNC_ID_BLE_MAC_SYNC);
    }
}
...
int bt_tws_connction_status_event_handler(int *msg)
{
    ...
    case TWS_EVENT_CONNECTED:
        ...
        app_send_message(APP_MSG_HJX_BLE_MAC, 0);
    ...
    case TWS_EVENT_CONNECTION_DETACH:
        ...
        //if(reason == TWS_DETACH_BY_LOCAL)
        {
            printf("----hjx_tws_dis_flag:%d\n",hjx_tws_dis_flag);
            if(hjx_tws_dis_flag == 1){
                break;
            }
        }
