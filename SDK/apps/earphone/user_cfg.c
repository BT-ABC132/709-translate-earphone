...
void cfg_file_parse(u8 idx)
{
    ...
    do {
        ret = syscfg_read(HJX_VM_BLE_MAC_INFO, mac_buf, 6);
        if ((ret != 6) || !memcmp(mac_buf, mac_buf_tmp, 6) || !memcmp(mac_buf, mac_buf_tmp2, 6)) {
            get_random_number(mac_buf, 6);
            syscfg_write(HJX_VM_BLE_MAC_INFO, mac_buf, 6);
        }
    } while (0);
    log_info("----ble_mac:");
    put_buf(mac_buf, sizeof(mac_buf));
    memcpy(hjx_ble_mac_buf, mac_buf, 6);

    do {
        ret = syscfg_read(HJX_VM_BLE_MAC2_INFO, mac_buf, 6);
        if ((ret != 6) || !memcmp(mac_buf, mac_buf_tmp, 6) || !memcmp(mac_buf, mac_buf_tmp2, 6)) {
            memset(hjx_ble_mac_buf2,0xff,6);
        }else{
            memcpy(hjx_ble_mac_buf2, mac_buf, 6);
        }
    } while (0);
    log_info("----TWS ble_mac:");
    put_buf(hjx_ble_mac_buf2, sizeof(hjx_ble_mac_buf2));
