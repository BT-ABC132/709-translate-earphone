...
static void tws_online_spp_in_task(u8 *data)
{
    ...
    case ONLINE_SPP_DATA:
        ...
        extern void app_data_receive_fun(u8 *buffer, u16 buffer_size);
        extern void hjx_app_data_deal();
        if (dual_bank_update_exist_flag_get() || classic_update_task_exist_flag_get()) {
            printf("---ota:%d %d\n",dual_bank_update_exist_flag_get(),classic_update_task_exist_flag_get());
        }else{
            app_data_receive_fun(&data[4], data_len);
            hjx_app_data_deal();
        }
#endif
