...
//------
#define ATT_LOCAL_PAYLOAD_SIZE    (517)//(247)//(517)                   //note: need >= 20
...
void hjx_check_connetion_updata_deal(hci_con_handle_t connection_handle)
{
...
void hjx_set_ble_work_state(ble_state_e state)
{
    rcsp_set_ble_work_state(state);
}
...
#if TCFG_USER_TWS_ENABLE && !defined HJX_TWS_TWO_BLE_EN
...
static void ble_auto_shut_down_enable(u8 enable)
{
    ...
    if (enable) {
        ...
        && (bt_rcsp_ble_conn_num() == 0)){    //已经没有设备连接
...
int rcsp_att_write_callback(hci_con_handle_t connection_handle, uint16_t att_handle, uint16_t transaction_mode, uint16_t offset, uint8_t *buffer, uint16_t buffer_size)
{
    ...
    case ATT_CHARACTERISTIC_FF01_01_VALUE_HANDLE:
        ...
        put_buf(buffer,buffer_size);
        app_data_receive_fun(buffer,buffer_size);
        hjx_app_data_deal();
    #if 0///原数据发回给APP
        hjx_custom_ble_data_send(buffer,buffer_size);
    #endif
    break;
    ...
    case ATT_CHARACTERISTIC_FF02_01_CLIENT_CONFIGURATION_HANDLE:
        ...
        hjx_check_connetion_updata_deal(connection_handle);
        att_set_ccc_config(att_handle, buffer[0]);
    break;
    default:
        break;
...
void rcsp_bt_ble_init(void)
{
    ...
    app_ble_set_mac_addr(rcsp_server_ble_hdl, hjx_ble_mac_buf);
    app_ble_set_mac_addr(rcsp_server_ble_hdl1, hjx_ble_mac_buf);
#endif
#endif
