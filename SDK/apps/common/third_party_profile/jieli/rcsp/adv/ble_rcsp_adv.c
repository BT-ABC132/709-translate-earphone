...
int rcsp_make_set_adv_data(void)
{
    ...
    rcsp_adv_fill_mac_addr(mac_buf);
    offset += make_eir_packet_data(&buf[offset], offset, HCI_EIR_DATATYPE_MANUFACTURER_SPECIFIC_DATA, mac_buf, sizeof(mac_buf));
#else
    le_controller_get_mac(mac_buf_2);
    user_buf[0]=mac_buf_2[5];
    user_buf[1]=mac_buf_2[4];
    user_buf[2]=mac_buf_2[3];
    user_buf[3]=mac_buf_2[2];
    user_buf[4]=mac_buf_2[1];
    user_buf[5]=mac_buf_2[0];
    //user_buf[6]=tws_api_get_role();
    user_buf[6]=hjx_flag_info.tws_channel;
    printf("--hjx_flag_info.tws_channel:0x%x \n",hjx_flag_info.tws_channel);
    offset += make_eir_packet_data(&buf[offset], offset, HCI_EIR_DATATYPE_MANUFACTURER_SPECIFIC_DATA, user_buf, 7);
    ...
    if (tws_api_get_tws_state()) {
        ...
        if (bt_rcsp_device_conn_num() < rcsp_max_support_con_dev_num()) {
            if (role == TWS_ROLE_MASTER) {
                ...
                rcsp_bt_ble_adv_enable(0);
            #else
				rcsp_bt_ble_adv_enable(1);///从机也要广播BLE
			#endif
            }
        }
    }
